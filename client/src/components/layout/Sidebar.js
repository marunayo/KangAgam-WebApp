import React from 'react';
import { Link, NavLink, useNavigate } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';
import { motion } from 'framer-motion';
import { useTheme } from '../../context/ThemeContext'; // Import useTheme

// Path ke logo (pastikan sesuai struktur folder Anda)
const logo = '/assets/images/logo-kang-agam.png';

// Komponen-komponen ikon internal untuk navigasi
const HomeIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>;
const BookIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>;
const UsersIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M15 21a6 6 0 00-9-5.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-3-5.197" /></svg>;
const ChartIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>;
const LogoutIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>;
const UserCircleIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0z" /></svg>;
// Ikon Kamus Budaya (saat ini tidak digunakan)
// const MuseumIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 14v3m4-3v3m4-3v3M3 21h18M3 10h18M3 7l9-4 9 4M4 10v11h16V10" /></svg>;

// Komponen-komponen ikon internal untuk Theme Switcher
const SunIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" /></svg>;
const MoonIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>;
const SunsetIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 10V3M15.24 12.24l4.95-4.95M10 21h4M3.5 13.5h17M12 17.5c-3.866 0-7-1.567-7-3.5s3.134-3.5 7-3.5 7 1.567 7 3.5-3.134 3.5-7 3.5z" /></svg>;

/**
 * Komponen internal Theme Switcher khusus untuk Sidebar Admin.
 * Mengganti tema secara berurutan (light -> dark -> sunset -> light).
 */
const AdminThemeSwitcher = () => {
    const { theme, changeTheme } = useTheme(); // Gunakan hook useTheme
    const themes = ['light', 'dark', 'sunset']; // Urutan tema
    
    // Handler untuk mengganti tema ke tema berikutnya dalam urutan
    const handleThemeChange = () => {
        const currentIndex = themes.indexOf(theme);
        const nextIndex = (currentIndex + 1) % themes.length; // Loop kembali ke awal
        changeTheme(themes[nextIndex]);
    };
    
    // Fungsi untuk merender ikon tema yang sesuai
    const renderIcon = () => {
        if (theme === 'dark') return <MoonIcon />;
        if (theme === 'sunset') return <SunsetIcon />;
        return <SunIcon />; // Default ke light
    };
    
    return (
        // Tombol dengan animasi menggunakan framer-motion
        <motion.button
            onClick={handleThemeChange}
            className="p-3 rounded-lg text-text-secondary hover:bg-background"
            title="Ganti Tema"
            whileHover={{ scale: 1.1 }} // Animasi saat hover
            whileTap={{ scale: 0.9 }} // Animasi saat diklik
            key={theme} // Kunci unik agar animasi berjalan saat tema berubah
            initial={{ opacity: 0, rotate: -90, scale: 0.5 }} // Animasi masuk
            animate={{ opacity: 1, rotate: 0, scale: 1 }} // Animasi selesai
            transition={{ duration: 0.3 }} // Durasi animasi
        >
            {renderIcon()}
        </motion.button>
    );
};


/**
 * Komponen Sidebar navigasi untuk admin di tampilan desktop (hidden lg:flex).
 */
const Sidebar = () => {
    const { logout } = useAuth(); // Fungsi logout dari AuthContext
    const navigate = useNavigate(); // Hook untuk navigasi

    // Handler untuk logout
    const handleLogout = () => {
        logout(); // Panggil fungsi logout
        navigate('/admin/login'); // Redirect ke halaman login admin
    };

    // Kelas CSS dasar untuk link navigasi
    const navLinkClasses = "p-3 rounded-lg text-text-secondary hover:bg-background";
    // Kelas CSS untuk link navigasi yang aktif
    const activeLinkClasses = "p-3 rounded-lg bg-primary/10 text-primary";

    return (
        // Container sidebar (fixed position, hanya tampil di lg ke atas)
        <div className="hidden lg:flex w-24 bg-background-secondary h-screen flex-col items-center justify-between py-6 shadow-lg fixed">
            {/* Bagian Atas: Logo dan Navigasi Utama */}
            <div>
                <Link to="/admin/dashboard"> {/* Logo mengarah ke dashboard */}
                    <img src={logo} alt="Logo" className="h-12 w-auto" />
                </Link>
                <nav className="flex flex-col items-center mt-12 space-y-6">
                    {/* Gunakan NavLink agar style 'active' bisa diterapkan */}
                    <NavLink to="/admin/dashboard" className={({ isActive }) => isActive ? activeLinkClasses : navLinkClasses} title="Dashboard">
                        <HomeIcon />
                    </NavLink>
                    <NavLink to="/admin/manage-topics" className={({ isActive }) => isActive ? activeLinkClasses : navLinkClasses} title="Kelola Topik">
                        <BookIcon />
                    </NavLink>
                    {/* Link Kamus Budaya (saat ini dikomentari) */}
                    {/* <NavLink to="/admin/manage-culture-topics" className={({ isActive }) => isActive ? activeLinkClasses : navLinkClasses} title="Kamus Budaya">
                        <MuseumIcon />
                    </NavLink> */}
                    <NavLink to="/admin/manage-admins" className={({ isActive }) => isActive ? activeLinkClasses : navLinkClasses} title="Kelola Admin">
                        <UsersIcon />
                    </NavLink>
                    <NavLink to="/admin/manage-learners" className={({ isActive }) => isActive ? activeLinkClasses : navLinkClasses} title="Kelola Pengguna">
                        <UserCircleIcon />
                    </NavLink>
                    <NavLink to="/admin/statistics" className={({ isActive }) => isActive ? activeLinkClasses : navLinkClasses} title="Statistik">
                        <ChartIcon />
                    </NavLink>
                </nav>
            </div>
            
            {/* Bagian Bawah: Theme Switcher dan Tombol Logout */}
            <div className="flex flex-col items-center gap-4">
                <AdminThemeSwitcher /> {/* Komponen pemilih tema */}
                <button onClick={handleLogout} className="p-3 rounded-lg text-red-500 hover:bg-red-500/10" title="Logout">
                    <LogoutIcon />
                </button>
            </div>
        </div>
    );
};

export default Sidebar;