        ...
        <div>
            <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    <div className="relative flex-grow">
                        ...
                        <input type="text" placeholder="Cari entri..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 pr-4 py-2 w-full border rounded-lg bg-background-secondary" />
                    </div>
                     <Link to="/admin/manage-culture-topics" className="bg-background-secondary text-text-secondary font-bold px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-gray-200 dark:hover:bg-gray-700 justify-center text-sm border border-gray-300 dark:border-gray-600">
                        <span>Kembali</span>
                    </Link>
                    {selectedEntries.length > 0 && (
                        <button onClick={() => setDeleteModal({ multi: true })} className="bg-red-500 text-white font-bold px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-red-600 text-sm">
                            <TrashIcon />
                            <span>Hapus ({selectedEntries.length})</span>
                        </button>
                    )}
                    <button onClick={() => setFormModal({ isOpen: true, mode: 'add', data: null })} className="bg-primary text-white font-bold px-4 py-2 rounded-lg flex items-center gap-2 hover:opacity-90 flex-shrink-0 text-sm">
                        <PlusIcon />
                        <span>Tambah</span>
                    </button>
                </div>
            ...
            <div className="bg-background-secondary rounded-xl shadow-md overflow-hidden">
                                ...
                                {isLoading ? (
                                    ...
                                    <>
                                        {currentItems.map((entry, index) => (
                                            <tr key={entry._id} className={`border-b border-background ${selectedEntries.includes(entry._id) ? 'bg-primary/10' : ''}`} style={{ height: '60px' }}>
                                                ...
                                                <td className="p-3 px-6 font-semibold text-text">{getLocalizedText(entry.title)}</td>
                ...
                <div className="p-4 border-t border-background">
                    <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} totalItems={filteredEntries.length} />
                </div>
            </div>
